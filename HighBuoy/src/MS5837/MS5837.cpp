...
bool MS5837::init() {
	...
	_wire->beginTransmission(MS5837_ADDR);
	_wire->write(MS5837_RESET);
	...
	for ( uint8_t i = 0 ; i < 7 ; i++ ) {
		_wire->beginTransmission(MS5837_ADDR);
		_wire->write(MS5837_PROM_READ+i*2);
		_wire->endTransmission();

		_wire->requestFrom(MS5837_ADDR,2);
...
void MS5837::read() {
	...
	_wire->beginTransmission(MS5837_ADDR);
	_wire->write(MS5837_CONVERT_D1_8192);
	_wire->endTransmission();

	delay(20); // Max conversion time per datasheet

	_wire->beginTransmission(MS5837_ADDR);
	_wire->write(MS5837_ADC_READ);
	_wire->endTransmission();

	_wire->requestFrom(MS5837_ADDR,3);
	D1_pres = 0;
	D1_pres = _wire->read();
	D1_pres = (D1_pres << 8) | _wire->read();
	D1_pres = (D1_pres << 8) | _wire->read();

	// Request D2 conversion
	_wire->beginTransmission(MS5837_ADDR);
	_wire->write(MS5837_CONVERT_D2_8192);
	_wire->endTransmission();
